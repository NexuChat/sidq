...
demo-ingest:
	docker run --rm --network host -e DATAHUB_TELEMETRY_ENABLED=false -v "$(CURDIR)/demo/ingest.dhub.yaml:/ingest.dhub.yaml:ro" $(DEMO_INGEST_IMAGE) ingest run -c /ingest.dhub.yaml --no-spinner --no-progress
