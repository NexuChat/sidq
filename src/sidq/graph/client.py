...
class MCPGraphClient:
    ...
    def _search(self, query: str) -> Any:
        ...
        return self._tool_caller(
            "search", {"query": query, "filter": "entity_type = dataset", "num_results": 50}
        )
    ...
    def _get_lineage_paths_between(
        ...
        return self._tool_caller(
            "get_lineage_paths_between", arguments
        )
    ...
    def find_dataset(self, name_or_urn: str) -> str | None:
        ...
        for item in _items(self._search(name_or_urn)):
            ...
            urn = _string(_mapping(item.get("entity")), "urn", "entity_urn", "entityUrn")
    ...
    def get_dataset(self, urn: str) -> DatasetInfo | None:
        ...
        result = DatasetInfo(
            ...
            terms=tuple(sorted(set(_strings(entity, "glossaryTerms", "terms", "term_urns", "termUrns")))),
            owners=tuple(sorted(set(_strings(entity, "ownership", "owners", "owner_urns", "ownerUrns")))),
    ...
    def paths_between(
        ...
        default_granularity = "column" if metadata.get("pathType") == "column-level" else "table"
...
class StdioMCPToolCaller:
    ...
    def __init__(self, command: str | None = None, *, gms_url: str | None = None) -> None:
        self._command = command or str(
            FilePath(__file__).resolve().parents[3] / ".venv" / "bin" / "mcp-server-datahub"
        )
        self._gms_url = gms_url or os.environ.get("DATAHUB_GMS_URL", "http://localhost:8080")
        self._requests: queue.Queue[tuple[str, Mapping[str, Any], Future[Any]] | None] = queue.Queue()
    ...
    def _start(self) -> None:
        if self._thread is not None:
            return
        self._thread = threading.Thread(target=self._run, name="sidq-datahub-mcp", daemon=True)
        self._thread.start()
    ...
    def _run(self) -> None:
        ...
        except BaseException as error:  # relay startup and pending-call failures to the caller
    ...
    async def _serve(self) -> None:
        ...
        async with stdio_client(StdioServerParameters(command=self._command, env=env)) as (read, write):
...
def _items(value: Any) -> list[Mapping[str, Any]]:
    ...
    for key in ("results", "entities", "items", "data", "schema_fields", "fields", "paths", "searchResults"):
...
def _first_entity(value: Any, urn: str) -> Mapping[str, Any] | None:
    ...
    if urn in document and isinstance(document[urn], Mapping) and "error" not in document[urn]:
        return _mapping(document[urn])
    if "error" not in document and _string(document, "urn", "entity_urn", "entityUrn") == urn:
        return document
    for entity in _items(value):
        if "error" not in entity and _string(entity, "urn", "entity_urn", "entityUrn") in (None, urn):
            return entity
    return None
...
def _parse_fields(value: Any) -> list[SchemaField]:
    ...
    for field in _items(value):
        ...
        if not path:
            continue
        native_type = _string(field, "native_type", "nativeType", "nativeDataType", "type", "data_type", "dataType") or ""
...
def _parse_lineage(value: Any, *, requested_column: str | None) -> LineageResult:
    ...
    for item in items:
        ...
        urn = _string(entity, "urn", "entity_urn", "entityUrn") or _string(item, "urn", "entity_urn", "entityUrn", "destination_urn", "destinationUrn")
        if urn:
            urns.append(urn)
            entity_type = _string(entity, "type", "entity_type", "entityType") or _string(item, "entity_type", "entityType", "type")
            if entity_type:
                entity_types[urn] = entity_type
            tags[urn] = tuple(sorted(set(_strings(entity, "tags", "tag_urns", "tagUrns"))))
    columns = {
        urn: tuple(sorted(str(column) for column in item.get("lineageColumns", ()) if isinstance(column, str)))
        for item in items
        if (urn := _string(_mapping(item.get("entity")), "urn"))
    }
    metadata = _mapping(document.get("metadata"))
    granularity = "column" if metadata.get("queryType") == "column-level-lineage" else "table"
...
def _parse_paths(value: Any, *, default_granularity: str = "table") -> list[LineagePath]:
