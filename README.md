...
| **Technical execution** | 1,171 tests, lint, format and types in one gate; the flagship `BLOCK` re-derives **byte-identical** from committed evidence. Published numbers are guarded — a stale one fails the build. | `make check` · `make gate-demo` |
...
| 2 | `make check` | Python 3.12; package downloads on first use | 1171 tests, lint, format, types — 1170 passed, 1 optional integration skipped, with 84.07% branch coverage; the same gates CI runs. | ~70s after bootstrap |
