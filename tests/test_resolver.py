...
def test_dbt_manifest_is_the_first_resolution_strategy(tmp_path) -> None:
    ...
    sql_file.write_text("SELECT customer_id, total FROM raw.customers", encoding="utf-8")
...
def test_naming_convention_resolves_a_non_dbt_file(tmp_path) -> None:
    ...
    assert result.touched_assets[0].urn == "urn:li:dataset:(urn:li:dataPlatform:snowflake,finance.payments,PROD)"
...
def test_explicit_map_resolves_a_non_dbt_path(tmp_path) -> None:
    ...
    assert result.touched_assets[0].urn == "urn:li:dataset:(urn:li:dataPlatform:postgres,raw.customer,PROD)"
...
def test_unresolvable_file_produces_evidence_without_raising(tmp_path) -> None:
    ...
    assert [(item.kind, item.subject) for item in result.evidence] == [("unresolved_asset", "notes/plan.txt")]
