...
`docs/RECONCILE-COVERAGE.md`, regenerated by `scripts/measure_reconcile.py`:
per source, per constraint kind — enumerated, represented, verdict distribution,
and the count of false accusations, which must read `0`.

The number that gets published is coverage, and it is expected to be 100% by
construction. Precision — the T0–T2 share versus T4 abstentions — is the honest
quality metric, and it is published unrounded.
