...
| `sidq audit --write-receipts --write-assertions` | The verdict of a receipt DataHub already acknowledged | Supported DataHub Python SDK, because the official MCP server registers no assertion mutation tool. **Not the runbook environment:** `acryl-datahub` conflicts with Sidq's `mcp>=2` over `pydantic`, so this path runs only from an interpreter that already carries the SDK, and refuses elsewhere by name | **Yes, explicit and opt-in, and it requires `--write-receipts`** | That DataHub accepted a native assertion definition and run event mirroring an already-written receipt, and returned them from the same GraphQL query its own UI issues. The assertion reports the verdict; it is never an input to it, and a receipt whose write failed is not mirrored. Measured 2026-08-09, evidence in `examples/06-native-assertion/`. No screenshot of the rendered tab was captured. |
