...
| `sidq audit --write-receipts --write-assertions` | The verdict of a receipt DataHub already acknowledged | Supported DataHub Python SDK, because the official MCP server registers no assertion mutation tool | **Yes, explicit and opt-in, and it requires `--write-receipts`** | That DataHub accepted a native assertion definition and run event mirroring an already-written receipt. The assertion reports the verdict; it is never an input to it, and a receipt whose write failed is not mirrored. |
